        ...
        for m_i in range(mention_num):  # m_i is the mention number
            ...
            for m_j in range(mention_num):  #
                ...
                indices_j = [
                    ...
                    for offset in range(nodes[sent_no][m_j][1] - nodes[sent_no][m_j][0])
                ]
                for index_i in indices_i:
                    for index_j in indices_j:
                        try:
                            sdp_path = nx.shortest_path(
                                graph, source="{0}".format(index_i), target="{0}".format(index_j)
                            )
...
def Init(data_file_name, rel2id, max_length=512, is_training=True, suffix=""):
    ...
    node_position_sent = np.zeros(
        (sen_tot, MAX_SENT_NUM, MAX_NODE_PER_SENT, MAX_SENT_LEN), dtype=np.int16
    )
    ...
    for i in range(len(ori_data)):
        ...
        node_num[i] = GetNodePosition(
            ori_data[i],
            node_position[i],
            node_position_sent[i],
            node_sent_num[i],
            entity_position[i],
            Ls,
        )
    ...
    save_object(
        os.path.join(out_path, name_prefix + suffix + "_node_position_sent.pkl"),
        node_position_sent,
    )
    save_object(os.path.join(out_path, name_prefix + suffix + "_node_num.pkl"), node_num)
    save_object(
        os.path.join(out_path, name_prefix + suffix + "_entity_position.pkl"), entity_position
    )
    ...
    save_object(
        os.path.join(out_path, name_prefix + suffix + "_structure_mask.pkl"), structure_mask
    )
    ...
    # np.save(
    #     os.path.join(out_path, name_prefix + suffix + "_node_position_sent.npy"),
    #     node_position_sent,
    # )
    ...
    # np.save(os.path.join(out_path, name_prefix + suffix + "_structure_mask.npy"), structure_mask)
